...
6. Choose R1 by default. R1 may use direct zero-model HelioTerm for repository I/O; that does not set the result's model-terminal fields. Choose R2 only when a model-backed terminal is explicitly requested. R2 uses one reusable Luna/high `heliolune_helioterm`; Spark is not an active binding. Allow at most 8 requests, 4 calls/request, 64 request bytes, and 256 response bytes. HelioTerm remains an observation leaf.
