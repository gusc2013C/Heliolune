...
import { activityAwareGraceMs, compactSteerPrompt, compactSynthesisPrompt, finalizationSchedule, shouldAttemptSynthesis } from "../plugins/luna-pool-orchestrator/scripts/finalization.mjs";
...
test("reserves a bounded synthesis window inside the original deadline", () => {
  ...
  assert.equal(schedule.steerGraceMs, 13_333);
...
test("supports an explicit reserve without extending the hard deadline", () => {
  ...
  assert.equal(schedule.steerGraceMs, 18_333);
...
test("starts a fallback turn only for invalid structured output", () => {
  ...
  assert.equal(shouldAttemptSynthesis({ code: "TURN_HARD_TIMEOUT", missingCompletion: true }, schedule, 45_000), true);
...
test("active missing-completion work receives bounded synthesis grace", () => {
  assert.equal(activityAwareGraceMs({ timeoutSeconds: 300, snapshot: { eventCount: 100, silentMs: 1000 } }), 30_000);
  assert.equal(activityAwareGraceMs({ timeoutSeconds: 300, snapshot: { eventCount: 100, silentMs: 60_000 } }), 0);
  assert.equal(activityAwareGraceMs({ timeoutSeconds: 600, snapshot: { eventCount: 100, silentMs: 1000 } }), 0);
});
