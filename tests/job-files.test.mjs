...
test("detached runner requests are atomic, scoped, and removable", async (t) => {
  ...
  t.after(() => rm(root, { recursive: true, force: true }));
  const request = { version: "0.7.0-alpha.2", kind: "task", args: { objective: "bounded" } };
...
test("runner request claim is atomic and admits only one owner", async (t) => {
  ...
  t.after(() => rm(root, { recursive: true, force: true }));
  const request = { version: "0.7.0-alpha.2", kind: "task", args: { objective: "once" } };
...
test("claim metadata becomes visible only after its JSON is complete", async (t) => {
  ...
  t.after(() => rm(root, { recursive: true, force: true }));
  await writeJobRequest(jobId, { version: "0.7.0-alpha.2", kind: "task", args: {} }, root);
  let publish;
  let staged;
